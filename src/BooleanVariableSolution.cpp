/*--------------------------------------------------------------------------*/
/*-------------------- File BooleanVariableSolution.cpp --------------------*/
/*--------------------------------------------------------------------------*/
/** @file
 * Implementation of the BooleanVariableSolution class.
 *
...
// register BooleanVariableSolution to the Solution factory

SMSpp_insert_in_factory_cpp_0( BooleanVariableSolution );
...
/// calls f() on a group, returning false if it is not one of BooleanVariable

template< class F >
bool on_group( const std::unique_ptr< BaseGroup > & group , F f )
{
 if( ( ! group ) || ( group->get_element_type() != typeid( BooleanVariable ) ) )
  return( false );
...
void BooleanVariableSolution::deserialize( const netCDF::NcGroup & group )
{
 ...
 if( ::deserialize< double >( group , "DynamicValues" ,
  ...
  if( ncVar.isNull() )
   throw( std::invalid_argument( "BooleanVariableSolution::deserialize: "
...
BooleanVariableSolution::~BooleanVariableSolution() {
 delete_vectors();
 }

/*--------------------------------------------------------------------------*/

void BooleanVariableSolution::delete_vectors() {
...
void BooleanVariableSolution::initialize( const Block * const block , bool read ) {
  this->delete_vectors();
...
void BooleanVariableSolution::initialize_static_variable_values
( const Block * const block , bool read ) {
 ...
 for( Vec_Group::size_type i = 0 ; i < static_variable_values.size() ; ++i ) {
  ...
  if( ! on_group( groups[ i ] ,
	...
	group.for_each_as< BooleanVariable >( [ & value ]( BooleanVariable & var ) {
	  *(value++) = var.get_value(); } );
	}
       } ) )
   throw( std::logic_error( std::string( "BooleanVariableSolution::initialize: "
...
void BooleanVariableSolution::initialize_dynamic_variable_values
( const Block * const block , bool read ) {
 ...
 for( Vec_Group::size_type i = 0 ; i < dynamic_variable_values.size() ; ++i ) {
  ...
  if( ! on_group( groups[ i ] ,
       ...
       if( read )
	group.for_each_cell_as< BooleanVariable >(
   ...
   throw( std::logic_error( std::string( "BooleanVariableSolution::"
...
void BooleanVariableSolution::apply_static( const Block * const block ,
                                        bool read ) {
 ...
 const auto & groups = block->get_static_variable_groups();
  ...
  if( groups.size() != static_variable_values.size() )
    throw( std::logic_error
     ( std::string
        ( "BooleanVariableSolution::apply_static: "
          "number of static Variable groups of this BooleanVariableSolution (" ) +
 ...
 for( Vec_Group::size_type i = 0 ; i < static_variable_values.size() ; ++i ) {
  ...
  if( ! on_group( groups[ i ] ,
	...
	group.for_each_as< BooleanVariable >( [ & value ]( BooleanVariable & var ) {
	  var.set_value( *(value++) >= 0.5 ); } );
       } ) )
   throw( std::logic_error( "BooleanVariableSolution::apply_static: "
			    "invalid types" ) );
  if( ! conforming )
   throw( std::logic_error( "BooleanVariableSolution::apply_static: the size of "
...
void BooleanVariableSolution::apply_dynamic( const Block * const block ,
                                         bool read ) {
 ...
 const auto & groups = block->get_dynamic_variable_groups();
  ...
  if( groups.size() != dynamic_variable_values.size() )
    throw( std::logic_error
     ( std::string
        ( "BooleanVariableSolution::apply_dynamic: "
          "number of dynamic Variable groups of this BooleanVariableSolution (" ) +
	 ...
	 if( read ) {
	  ...
	  for( auto & item : cell )
	   if( value != cell_values.end() )
	    group_element( item ).set_value( *(value++) >= 0.5 );
	   else
	    group_element( item ).set_to_default_value();
	  }
	 } );
       } ) )
   throw( std::logic_error( "BooleanVariableSolution::apply_dynamic: "
			    "invalid types" ) );
  if( ! conforming )
   throw( std::logic_error( "BooleanVariableSolution::apply_dynamic: the number "
...
void BooleanVariableSolution::read( const Block * const block ) {
  ...
  if( static_variable_values.size() != block->get_static_variable_groups().size() ||
    ...
    // This BooleanVariableSolution does not have the same structure as
  ...
  try {
    ...
    // The given Block and this BooleanVariableSolution do not have the
    // same structure. Initialize this BooleanVariableSolution so that it
  ...
  if( sub_blocks.size() != nested_solutions.size() )
      throw( std::logic_error
       ( std::string( "BooleanVariableSolution::read() "
...
void BooleanVariableSolution::write( Block * const block ) {
  ...
  if( sub_blocks.size() != nested_solutions.size() )
   throw( std::logic_error
    ( std::string( "BooleanVariableSolution::write: number of nested Blocks (" ) +
      std::to_string( sub_blocks.size() ) +
...
void BooleanVariableSolution::serialize( netCDF::NcGroup & group ) const
{
...
void BooleanVariableSolution::sum( const Solution * solution, double multiplier ) {

  auto other_solution = dynamic_cast< const BooleanVariableSolution * >( solution );

  if( ! other_solution )
    throw( std::invalid_argument
     ( "BooleanVariableSolution::sum: "
       "given Solution must be a BooleanVariableSolution" ) );
  ...
  if( ( static_variable_values.size() !=
    ...
    throw( std::logic_error
     ( std::string( "BooleanVariableSolution::sum: "
 ...
 for( Vec_Group::size_type i = 0 ; i < static_variable_values.size() ; ++i ) {
  ...
  if( values.size() != other_values.size() )
   throw( std::logic_error( "BooleanVariableSolution::sum: static variable group "
			    + std::to_string( i ) + " has " +
 ...
 for( Vec_Group::size_type i = 0 ; i < dynamic_variable_values.size() ; ++i ) {
  ...
  if( cells.size() != other_cells.size() )
   throw( std::logic_error( "BooleanVariableSolution::sum: dynamic variable group "
			    + std::to_string( i ) + " has " +
...
BooleanVariableSolution * BooleanVariableSolution::scale( double factor ) const {
  auto scaled_solution = new BooleanVariableSolution();
...
BooleanVariableSolution * BooleanVariableSolution::clone( bool empty ) const {
  auto cloned_solution = new BooleanVariableSolution();
...
void BooleanVariableSolution::scale( const BooleanVariableSolution * const solution ,
                                 const double factor ) {
...
/*--------------------------------------------------------------------------*/
/*------------------ End File BooleanVariableSolution.cpp ------------------*/
/*--------------------------------------------------------------------------*/

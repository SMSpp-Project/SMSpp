 ...
 if( changedvars ) {
  ...
  for( int k = 0 ; k < mat_nd.outerSize() ; ++k ) {
   for( Qmat::InnerIterator it( mat_nd , k ) ; it ; ++it ) {
    auto variable_value1 = std::get< 0 >( v_triples[ it.row() ] )->get_value();
    auto variable_value2 = std::get< 0 >( v_triples[ it.col() ] )->get_value();
...
bool QuadFunction::is_convex( void ) {
 ...
 case Unknown :
  throw( std::logic_error(
   "At this stage we should know what function this is" ) );
  break;
 default :
...
void QuadFunction::get_hessian_approximation( SparseHessian & hessian ) const {
 ...
 for( int k = 0 ; k < mat_nd.outerSize() ; ++k ) {
  for( Qmat::InnerIterator it( mat_nd , k ) ; it ; ++it ) {
   tripletList.push_back(
    Eigen::Triplet< FunctionValue >( it.row() , it.col() , it.value() ) );
...
void QuadFunction::identify_convexity( void ) {
 /**
    *
 ...
 if( my_convexity == Unknown ) {
  ...
  if( ( ldlt.info() != Eigen::NumericalIssue ) && ldlt.isPositive() ) {
   ...
   my_convexity = Concave;
           ...
           ( ( ! ldlt.isPositive() ) && ( ! ldlt.isNegative() ) ) ) {
   my_convexity = NotConvex;
  }
  else {
   throw( std::logic_error(
    "Symmetric matrices can not be something other than PSD, NSD or indefinite ... yet ..." ) );
  }
 }
}

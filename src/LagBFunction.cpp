...
void LagBFunction::set_dual_pairs(v_dual_pair &&dp)
{
    ...
    for (Index i = 0; i < dp.size() - 1; ++i)
      if (dp[sorted[i]].first == dp[sorted[i + 1]].first)
        throw(std::invalid_argument("LagBFunction::set_dual_pairs: repeated "
                                    ...
                                    " ] and dp[ " + std::to_string(sorted[i + 1]) + " ]"));
  }
#endif
  ...
  for (auto const &lp : LagPairs) // ... unless otherwise proven
    ...
    {
      f_yb = NaN;
      break; // if so, yb has to be computed
    }
  ...
  if (scfg)
    if (scfg->f_extra_Configuration)
    {
              ...
              scfg->f_extra_Configuration))
      {
        ...
        else if (!scfg->diff())                  // if not in differential mode
      ...
      else if (!(BC = dynamic_cast<BlockConfig *>(
                     scfg->f_extra_Configuration)))
      ...
      if (BSC)
      { // set the BlockSolverConfig of the inner Block, if any
        if (f_BSC)
        {
...
void LagBFunction::set_par(idx_type par, double value)
{
  ...
  case (dblAbsAcc):
    ...
    break;
  case (dblUpCutOff):
    add_par("dblUpCutOff", value);
    break;
  case (dblLwCutOff):
    add_par("dblLwCutOff", value);
    break;
  case (dblRAccLin):
    add_par("dblRAccSol", value);
    break;
  case (dblAAccLin):
    add_par("dblAAccSol", value);
  }
...
void LagBFunction::add_dual_pairs(v_dual_pair &&dp, ModParam issueMod)
{
  ...
  if (f_yb == -INF)
    for (auto const &el : dp)
      ...
      {
        f_yb = NaN;
        break; // if not, yb has to be computed
      }
...
void LagBFunction::remove_variables(Range range, ModParam issueMod)
{
  ...
  if (f_yb > -INF)
    for (auto lpit = strtit; lpit != stopit; ++lpit)
      ...
      {
        f_yb = INF;
        break; // if so, signal to check if b == 0 or not
      }
      ...
      for (Index h = 0; h < CostMatrix.size(); ++h)
      {
        ...
        for (auto &col : CMh)
        {
          ...
          for (auto itp = Aj.begin(); itp != Aj.end(); ++itp)
          {
            ...
            while ((nit != nms.end()) && (*nit < itp->first))
            {
              ++nit;
              ++cnt;
            }
...
void LagBFunction::cleanup_inner_objective(void)
{
  ...
  for (Index h = 0; h < CostMatrix.size(); ++h)
  {
    ...
    if (!v_ObjIsQuad[h])
      static_cast<p_LF>(v_Obj[h]->get_function())->modify_coefficients(std::move(NC));
    else
      static_cast<p_QF>(v_Obj[h]->get_function())->modify_linear_coefficients(std::move(NC));
  }
...
void LagBFunction::serialize(netCDF::NcGroup &group) const
{
  ...
  if (!f_c_changed)
  {                                 // if the costs are still the original ones
                                   ...
                                   const std::string &sub_group_name) const
{
  ...
  if (f_max_glob)
  {
    ...
    (group.addVar("LagBFunction_Type", netCDF::NcByte(), gs)).putVar({0}, {f_max_glob}, typ.data());
...
bool LagBFunction::has_linearization(bool diagonal)
{
  ...
  if (newlin)
  {                               // the Solver has the desired stuff
    ...
    c_LinearCombination &coefficients, Index name, ModParam issueMod)
{
      ...
      (v_active.size() == CostMatrix.size()))
  {
    ...
    for (const auto &cf : coefficients)
    {
      ...
      if (cca.size() != v_active.size())
      {
        ok = false;
        break;
      }
      for (Index h = 0; ok && (h < cca.size()); ++h)
        if (cca[h].size() != v_active[h].size())
        {
          ok = false;
          break;
        }
...
int LagBFunction::compute(bool changedvars)
{
  ...
  if (f_yb == INF)
  {
    ...
    for (auto const &lp : LagPairs)
      ...
      {
        f_yb = NaN;
        break; // if b has nonzeros, yb need be recomputed
      }
  }
  ...
  if (f_dirty_Lc)
  {
    ...
    for (Index h = 0; h < CostMatrix.size(); ++h)
    {
      ...
      for (Index i : v_active[h])
      {             // only coords coupled to a multiplier
                                          ...
                                          c_Subset &subset, Index name)
{
  ...
  for (Index k = 0; k < subset.size(); ++k)
  {
    ...
    pos[subset[k]] = k;
    g[k] = static_cast<p_LF>(LagPairs[subset[k]].second)->get_constant_term();
  }
                                                  ...
                                                  Range range, Index name)
{
  ...
  if (name == Inf<Index>())
  { // the last computed linearization- - - - - -
  ...
  { // a linearization of the global pool - - - - - - - - - - - - - - - -
    if (NoSol)
                                               ...
                                               ComputeConfig *ocfg) const
{
  ...
  { // else (the extra Configuration has to be constructed) - - - - - - -
...
bool LagBFunction::flush_v_tmpCP(void)
{
  ...
  if (!v_Block.front()->is_owned_by(f_id))
  {                                   // if the inner Block is free
                                                    ...
                                                    ChnlName chnl)
{
  ...
  if (const auto tmod = dynamic_cast<const C05FunctionModLinRngd *>(mod))
  {
    if (const auto lf = dynamic_cast<p_LF>(tmod->function()))
    {
      ...
      if (lf == CMh_f)
      { // ... inside the Objective of the inner Block - - - -
  ...
  if (const auto tmod = dynamic_cast<const FunctionMod *>(mod))
  {
    ...
    if (dynamic_cast<Objective *>(f->get_Observer()))
    {
      ...
      { // an unpredictable change in an Objective
        // issue a LagBFunctionMod modification of the type AlphaChanged and
        // with what() == 1: the Lagrangian function unpredictably changes
        // (f_shift == NaN), and the constant terms \alpha = c x^* of the
        // linearizations ( g , \alpha ) have to be computed again since
...
char LagBFunction::guts_of_this_add_Modification(p_Mod mod, ChnlName chnl)
{
  ...
  else if (auto fmodv = dynamic_cast<const FunctionModVars *>(mod))
                                                ...
                                                c_Range &rng)
{
  ...
  if (current)
  {                                                // if Range is still current, it's easy
                                                ...
                                                c_Subset &sbst)
{
  ...
  if (current)
  {                                                // if Subset is still current, it's easy
    std::vector<std::pair<Index, double>> jdeltas; // EAGER: (j, Delta_c_j)
    for (Index j : sbst)
    {
      if (!f_lazy_eval)
...
void LagBFunctionState::serialize(netCDF::NcGroup &group) const
{
  ...
  if (f_max_glob)
  {
    ...
    (group.addVar("LagBFunction_Value", netCDF::NcDouble(), gs)).putVar({0}, {f_max_glob}, val.data());
    (group.addVar("LagBFunction_Convexified", netCDF::NcByte(), gs)).putVar({0}, {f_max_glob}, cvx.data());

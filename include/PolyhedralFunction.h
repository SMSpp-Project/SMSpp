...
/*--------------------------------------------------------------------------*/
 /// build a closed-form Block representation of this PolyhedralFunction
 /** Concrete override of C05Function::build_easy_Block(): packages this
  * PolyhedralFunction
  *
  *     F( x ) = max_i { a_i . x + b_i }   (convex case;
  *                                         min for the concave case)
  *
  * inside a freshly-allocated PolyhedralFunctionBlock ready to be
  * grafted under the abstract Block tree of any caller (a Bundle Solver
  * managing it as an "easy" component, a Benders sub-problem master,
  * ...).
  *
  * The new sub-Block wraps a copy of (A, b) and shares the same active
  * ColVariable pointers, so any change to those Variable propagates to
  * both forms automatically. The convexity flag and the global bound of
  * this PolyhedralFunction are copied into the wrapped function as
  * well.
  *
  * @param primal  selects the abstract representation generated by the
  *                returned Block:
  *                - true  -> linearized-primal (rep == 1):
  *                            min  v
  *                            s.t. v >= a_i . x + b_i for every row i
  *                  giving a closed-form epigraph master ready for the
  *                  primal MP of a Bundle Solver;
  *                - false -> linearized-dual (rep == 3):
  *                            max  sum_i theta_i b_i + gamma * LB
  *                            s.t. sum_i theta_i + gamma = 1
  *                                 sum_i theta_i a_i = z
  *                                 theta_i, gamma >= 0
  *                  giving the standard simplex-on-the-rows dual.
  *
  * @return  a pointer to a freshly-allocated PolyhedralFunctionBlock
  *          owned by the caller. */

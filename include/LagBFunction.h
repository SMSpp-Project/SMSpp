...
namespace SMSpp_di_unipi_it
{
  ...
  public:
    ...
    enum event_type_lagBFunction
    {
       ...
       * instead of deleting valid column, move them in GlobalInformation
       */
      e_last_event_type
      ///< convenience value to define new events
      /**< convenience value to allow derived classes to "extend"
       * event_type_lagBFunction and define new class-specific events that their
       * compute() can support. */
     ...
     * Solver.
     *
     * This is true except for move_column_event, which is handled by LagBFunction itself in add_Modification()
     *
     *  @{ */
    ...
    EventID set_event_handler(int type, EventHandler &&event) override
    {
      ...
      {
        // Reuse eventual position freed by reset_event_handler().
        for (EventID id = 0;
             id < static_cast<EventID>(v_column_purged_handlers.size());
             ++id)
        {
          if (!v_column_purged_handlers[id])
          {
            v_column_purged_handlers[id] = std::move(event);
            return id;
          }
        }
        if (v_column_purged_handlers.size() > std::numeric_limits<EventID>::max())
          throw(std::invalid_argument(
              "LagBFunction: too many event handlers for eColumnPurged"));
        EventID id = v_column_purged_handlers.size();
        v_column_purged_handlers.push_back(std::move(event));
        return (id);
      }
      if (auto is = inner_Solver())
    ...
    void reset_event_handler(int type, EventID id) override
    {
      if (type == eColumnPurged)
      {
        if (id >= v_column_purged_handlers.size())
          throw(std::invalid_argument(
              "LagBFunction: incorrect event id for eColumnPurged"));
    ...
    [[nodiscard]] int get_int_par(idx_type par) const override
    {
      ...
      case (intPushCostToOwner):
        return (PushCostToOwner ? 1 : 0);
      case (intPoolExtMem):
        return (f_lazy_eval ? 1 : 0);
      }
    ...
    [[nodiscard]] double get_dbl_par(idx_type par) const override
    {
      ...
      if (auto is = inner_Solver())
        ...
        {
        case (dblRelAcc):
          return (is->get_dbl_par(Solver::dblRelAcc));
        case (dblAbsAcc):
          return (is->get_dbl_par(Solver::dblAbsAcc));
        case (dblUpCutOff):
          return (is->get_dbl_par(Solver::dblUpCutOff));
        case (dblLwCutOff):
          return (is->get_dbl_par(Solver::dblLwCutOff));
        case (dblRAccLin):
          return (is->get_dbl_par(Solver::dblRAccSol));
        case (dblAAccLin):
          return (is->get_dbl_par(Solver::dblAAccSol));
        }
        ...
        const override
    {
      ...
      while (varsit != vars.end())
      {
        ...
        // covers the entire LagPairs and Phase 2 below is unneeded.
        Index i = std::min(oi, Index(LagPairs.size() - 1));

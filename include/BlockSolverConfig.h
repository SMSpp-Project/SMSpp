...
namespace SMSpp_di_unipi_it
{
  ...
  public:
    ...
    explicit BlockSolverConfig(Block *block,
                               bool diff = false, bool clear = false, bool add = false)
        : Configuration(), f_diff(diff), add(add)
    {
    ...
    virtual void apply(Block *block, bool adding = false);
    ...
    void set_Solver_name(Index index, std::string &&name = "")
    {
      if (index >= v_SolverNames.size())
        throw(std::invalid_argument(
            "BlockSolverConfig::set_Solver_name: invalid index: " + std::to_string(index)));
                           ...
                           bool destroy = true)
    {
      if (index >= v_SolverConfigs.size())
        throw(std::invalid_argument(
            "BlockSolverConfig::set_ComputeConfig: invalid index: " + std::to_string(index)));
      if (destroy)
        delete v_SolverConfigs[index];
    ...
    /*--------------------------------------------------------------------------*/
    /// returns the pointers of all Solver  in this BlockSolverConfig
    /** This function returns a const reference to the vector containing the
     * Solver  in this BlockSolverConfig. */

    [[nodiscard]] const std::vector<Solver *> &get_Solvers(void)
        const { return (vp_Solvers); }
    /*--------------------------------------------------------------------------*/
    ...
    [[nodiscard]] const std::string &get_SolverName(Index index) const
    {
      if (index >= v_SolverConfigs.size())
        throw(std::invalid_argument(
            "BlockSolverConfig::get_SolverName: invalid index: " + std::to_string(index)));
  ...
  protected:
    ...
    /// the pointers to the created solvers (created during apply())
    std::vector<Solver *> vp_Solvers;
  ...
  public:
    ...
    void apply(Block *block, bool adding = false) override;
        ...
        Block::Index index) const
    {
      if (index >= v_BlockSolverConfig.size())
        throw(std::invalid_argument(
            "RBlockSolverConfig::get_BlockSolverConfig: invalid index: " + std::to_string(index)));
